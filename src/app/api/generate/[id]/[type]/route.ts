...
export async function GET(req: Request, { params }: { params: Promise<{ id: string, type: string }> }) {
  try {
    ...
    const tempDocxPath = path.join(os.tmpdir(), `temp_doc_${id}_${timestamp}.docx`);
    const tempPdfPath = path.join(os.tmpdir(), `temp_doc_${id}_${timestamp}.pdf`);

    // 1. Write the Docx buffer to disk
    fs.writeFileSync(tempDocxPath, buf);

    try {
      // 2. Convert to PDF using LibreOffice
      // Note: We run libreoffice in headless mode to convert the file in the background.
      await execAsync(`libreoffice --headless --convert-to pdf "${tempDocxPath}" --outdir "${os.tmpdir()}"`);
      ...
      // 4. Delete the temporary files to save disk space
      ...
      return new NextResponse(pdfBuf as any, {
        status: 200,
        headers: {
          "Content-Disposition": `attachment; filename="${consumer.consumerName.replace(/\s+/g, '_')}_${type}.pdf"`,
      ...
      // Cleanup files if they exist to prevent memory leaks even on error
      if (fs.existsSync(tempDocxPath)) fs.unlinkSync(tempDocxPath);
      if (fs.existsSync(tempPdfPath)) fs.unlinkSync(tempPdfPath);

      // Fallback: If LibreOffice fails, just return the Word document so they aren't totally stuck
